...
export default function NewRequestPage() {
    ...
    const { addRequest } = useFulfillment();

    const handleSave = (requestData: any) => {
        const id = "FUL-" + Math.floor(100 + Math.random() * 900);
        ...
        <div className="bg-background text-on-background h-screen overflow-hidden flex font-body-md">
            ...
            <main className="flex-1 overflow-y-auto bg-background relative z-0 ml-64 p-margin-desktop">
                <header className="mb-stack-lg flex justify-between items-end max-w-3xl">
                    <div>
                        <h2 className="font-headline-lg text-headline-lg text-on-background mb-2">New Fulfillment Request</h2>
                        <p className="font-body-md text-body-md text-on-surface-variant">Log a new vehicle inquiry or delivery request.</p>
                    </div>
                ...
                <FulfillmentForm onSubmit={handleSave} />
            </main>
        </div>
    );
}
